...
public class AnalyticsController : MonoBehaviour
{
    ...
    #region Practice Writing Trackers
    public void PostLevelStart()
    {
        if (!firebaseInitialized) return;
        FirebaseAnalytics.LogEvent("START_level_" + (UseProfile.CurrentLevel + 1));
    }

    public void PostQuitLevel()
    {
        if (!firebaseInitialized) return;
        FirebaseAnalytics.LogEvent("QUIT_level_" + (UseProfile.CurrentLevel + 1));
    }

    public void PostUsePowerUp(int id)
    {
        if (!firebaseInitialized) return;
        switch (id)
        {
            case 1:
                FirebaseAnalytics.LogEvent("BOOSTER_DELETE_MISTAKE_used_at_level_" + (UseProfile.CurrentLevel + 1));
                break;
            case 2:
                FirebaseAnalytics.LogEvent("BOOSTER_RESHUFFLE_used_at_level_" + (UseProfile.CurrentLevel + 1));
                break;
            case 3:
                FirebaseAnalytics.LogEvent("BOOSTER_INSTANT_ORDER_used_at_level_" + (UseProfile.CurrentLevel + 1));
                break;
        }
    }

    public void PostClaimBonus()
    {
        if (!firebaseInitialized) return;
        FirebaseAnalytics.LogEvent("End_Game_Bonus_CLAIMED_at_level_" + (UseProfile.CurrentLevel + 1));
    }

    public void PostWinLevel()
    {
        if (!firebaseInitialized) return;
        FirebaseAnalytics.LogEvent("WIN_at_level_" + (UseProfile.CurrentLevel + 1));
    }

    public void PostLoseLevel()
    {
        if (!firebaseInitialized) return;
        FirebaseAnalytics.LogEvent("LOSE_at_level_" + (UseProfile.CurrentLevel + 1));
    }

    public void PostWatchUnlimitedHeartAds()
    {
        if (!firebaseInitialized) return;
        FirebaseAnalytics.LogEvent("Player_watch_ads_for_UNLIMITED_HEART_at_level_" + (UseProfile.CurrentLevel + 1));
    }
    #endregion
